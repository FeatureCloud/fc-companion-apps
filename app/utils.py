...
def plot_clients_data(df, path):
    ...
    for v in df.ASSIGNED_CLIENT.unique()[:-1]:
        plt.axvline(x=int(v) + 0.5, color='black', linestyle='dotted')
...
def load_dataset(ds_conf, path):
    ...
    """
    if ds_conf['format'] in ['txt', 'csv']:
        df = pd.read_csv(path + '/' + ds_conf['filename'], sep=ds_conf['sep'])
        if ds_conf['task'] == "classification":
            df = df.rename(columns={ds_conf['target_value']: 'label'})
            print(df.columns)
        else:
            print('Unsupervised or Regression dataset')
    else:  # NumPy
        ds = np.load(path + '/' + ds_conf['filename'], allow_pickle=True)
        if ds_conf['task'] == 'classification':
            if ds_conf['target_value'] == 'same-sep':
                df = pd.DataFrame({"features": [s for s in ds[0]], "label": ds[1]})
            elif ds_conf['target_value'] == 'same-last':
                df = pd.DataFrame({"features": [s[:-1] for s in ds], "label": [s[-1] for s in ds]})
            elif ds_conf['target_value'].strip().split(".")[1].lower() == 'npy':
                labels = np.load(path + '/' + ds_conf['filename'], allow_pickle=True)
                df = pd.DataFrame({"features": [s for s in ds], "label": labels})
            else:
                raise ValueError(f"Oops! {ds_conf['target_value']} is not a supported value for target_value.")
